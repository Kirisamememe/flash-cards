...
>({href, className, disabled, ...rest}: ComponentProps<typeof Link<Pathname>> & { disabled?: boolean }) {
    ...
    return (
        <Button disabled={disabled} asChild className={cn("h-full rounded-none active:bg-transparent active:text-primary", className)} variant={"ghost"}
                onClick={(event) => {event.currentTarget.animate(bounceAnimation, bounceTiming)}}
        >
            <Link
