            ...
            <div className={"flex w-full gap-3 justify-between items-start"}>
                <FormItem className={"w-full"}>
                    ...
                    <FormControl>
                        ...
                        >
                            <SelectTrigger disabled={poses.length <= 0} className={cn("w-full h-12 px-5 rounded-lg border-0 ring-1 ring-border hover:bg-muted/20 focus:ring-offset-0 focus:ring-2 focus:outline-none font-semibold", !fieldValue && "text-foreground/40")}>
                                <SelectValue
                ...
                <Button className={"h-12"} onClick={() => setIsAddingPos(true)} variant={"coloredOutline"}
                        type={"button"}>{t("add")}
                </Button>
            </div>
        </>
